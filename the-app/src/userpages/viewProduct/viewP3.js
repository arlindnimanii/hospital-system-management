...
function ViewP3() {
  ...
  function getproduct3() {
    axios
      .get(`http://127.0.0.1:8000/api/product3s/${id}`)
      .then(function (response) {
        console.log(response.data);
        setproduct3(response.data.product3);
        setInputs(response.data.product3);
      })
  ...
  const handleQtyChange = (e) => {
    ...
    const maxQty = product3.qty; // Maximum available stock
  ...
  const handleSubmit = (e) => {
    ...
    // Navigate to the cart view page with the product3 data
  ...
  return (
    <div className="product3 py-5">
      <div className="container">
        <div className="row mt-5">
          <div className="col-lg-6 col-md-6 col-sm-12">
            <h1>{inputs.name}</h1>
            <img src={`http://127.0.0.1:8000/storage/product3s/${inputs.image}`} alt={inputs.name} style={{ width: '250px', height: '250px' }} />
